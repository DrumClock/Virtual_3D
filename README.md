...
- **MULTI HOTEND** 
 ...
 automatická výmìna filamentu mimo tiskovou ploxchu.
...
- **ENDLESS SPOOL** - nekoneèná cívka (oznaèovaný jako **Backup mode** )

 Pokud je detekován konec filamentu pomocí sensoru (switch nebo motion), dojde k výmìnì za extruder který má filament.
  
 ZároveÅˆ jsou všechny tiskové G-cody  **`Tx`** smìrovány na nový extruder a to vèetnì  **`M104 / M109`**.
 
 Proavidlo pro náhradu extruderu je následující:  **extruder > extruder1 > extruder2 > extruder3 > etc**
 
 První extruder který má detekovaný filament se pouÅ¾ije.
...
- **DUAL GANTRY** (dvojitý portál) - nová kinematika  **`dualgantry_cartesian`** 
 ...
 Je typ kinematiky pro dvì nástrojové hlavy, které se pohybují pomocí dvou zcela nezávislých os XY.
...
Všechny uvedené sloÅ¾ky a soubory jsou v Raspberry uloÅ¾ené zde:  **`/home/pi/my_config/`**

Pro zobrazení sloÅ¾ky **`my_config`** v rozhranní **Mainsail** je nutné vytvoÅ™it "soft-link" pomocí:

**` ln -s /home/pi/my_config /home/pi/xxx/my_config `**   (xxx - adresáÅ™ s cfg napÅ™. klipper_config)

# Makra jsou univerzální pro tyto konfigurace

- single carriage pro 2 aÅ¾ 4 extrudery
- dual carriige (IDEX) pro 2 aÅ¾ 4 extrudery

# Popis adresáÅ™Å¯:
**Klipper zatím nepodoruje - moduly pro testování poskytl Tircown**
- **`IDEX`**          - M605 mody - DUPLICATION a MIRRORED 
- **`4EX2`**          - rozšíÅ™ení IDEX modu M605 pro 4 extrudery (dvoubarevný tisk DUPLICATION a MIRRORED)
...
- **`MACRO`**         - rozšíÅ™ená univerzální makra  
...
# Jednotlivá makra se naèítají pomocí [include ...] napÅ™.
...
# Promìnné pro makra se vytvoÅ™í pomocí RUN_MACRO_INIT 
spuštìného v [delayed_gcode _INIT] z konfigurace tiskárny a to po restartu FW napÅ™.
